...
test('homepage renders the live hub sections on mobile', async ({ page }) => {
  ...
  await expect(page.getByText(/broadcast editorial briefing/i)).toBeVisible();
  ...
  await expect(page.getByRole('tabpanel', { name: /east standings/i })).toBeVisible();
  await expect(page.getByText(/bracket studio with seed paths, momentum cards, and star watch/i)).toBeVisible();
});
